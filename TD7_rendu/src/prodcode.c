...
static void produce_declarations(ast_node* node, int emit_init)
{
    ...
    FORLIST(p, n->vars)
    {
        ...
        code(var);
        if (emit_init)
        {      // Initialization code must be issued
            emit(" = ");
            if (val)
                code_expr_cast(val);
            else
                emit(TYPE_DEFAULT_INIT(AST_TYPE(node)));
        }
        if (--len)
            emit(", ");
    }
...
void produce_code_call(ast_node* node)
{
    ...
    if (!AST_TYPE(node))
        return; // previous error detected
...
void produce_code_print_statement(ast_node* node)
{
    ...
    FORLIST(p, n->parameters)
    {
        ...
        if (!first_parameter)
            indent(0);
        if (AST_TYPE(item) == int_type)
        {                           // int
            emit("_toy_print_int(");
        }
        else if (AST_TYPE(item) == float_type)
        {                  // float
            emit("_toy_print_float(");
        }
        else if (AST_TYPE(item) == bool_type)
        {                   // bool
