 ...
 * Last file update:  6-Dec-2021 18:45 (eg)
 */
...
// return a Toy type as a C string
static char *toy_type_to_string(ast_node *type) {
  if      (type == int_type)    return "int";
  else if (type == float_type)  return "float";
  else if (type == bool_type)   return "bool";
  else if (type == string_type) return "string";
  else if (type == void_type)   return "void";
...
void produce_code_string_access(ast_node *node) {
  ast_node *name = STRING_ACCESS_STR(node);
  ast_node *idx1 = STRING_ACCESS_INDEX1(node);
  ast_node *idx2 = STRING_ACCESS_INDEX2(node);

  emit("_toy_str_");
  if (STRING_ACCESS_SLICE(node)) {
    // We have a slice 
    emit("slice("); code(name); emit(", ");

    // Emit idx1 (or 0 if idx is NULL)
    if (!idx1) emit("UNSPECIFIED_SLICE_LIMIT"); else code(idx1);
    emit(", ");

    // Emit idx2 (or MAX_INT
    if (!idx2) emit("UNSPECIFIED_SLICE_LIMIT"); else code(idx2);
  } else {
    emit("element(");
    code(name); emit(", ");
    code(idx1);
...
void produce_code_print_statement(ast_node *node) {
  ...
  FORLIST(p, n->parameters) {
    ...
    // produce "_toy_print_XXX(<code of item>)" where XXX is string, int, bool …
    emit("_toy_print_%s(", toy_type_to_string(AST_TYPE(item)));
    code(item);
    emit(");\n");
