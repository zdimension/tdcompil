 ...
 * Last file update:  6-Dec-2021 19:41 (eg)
 */
...
void analysis_string_access(ast_node *node) {
  ...
  analysis(STRING_ACCESS_INDEX1(node));
  analysis(STRING_ACCESS_INDEX2(node));

  if (AST_TYPE(STRING_ACCESS_STR(node)) != string_type)
    error_msg(node, "bad string");

  if (STRING_ACCESS_INDEX1(node) && AST_TYPE(STRING_ACCESS_INDEX1(node)) != int_type)
    error_msg(node, "low string index must be of integer type");

  if (STRING_ACCESS_INDEX2(node) && AST_TYPE(STRING_ACCESS_INDEX2(node)) != int_type)
    error_msg(node, "upper string index must be of integer type");
...
void analysis_return_statement(ast_node *node) {
  ...
  n->parent_function = current_function;
  analysis(n->expr);
