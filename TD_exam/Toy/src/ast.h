 ...
 * Last file update:  3-Dec-2021 09:48 (eg)
 */
...
enum node_kind {k_constant, k_identifier, k_type, k_var_decl, k_expression,
                ...
                k_foreach_statement, k_block_statement, k_expr_statement,
                k_print_statement, k_return_statement, k_break_statement,
                k_string_access};
...
#define IDENT_IS_CONST(p)       (((struct s_identifier *) p)->is_const)
...
struct s_var_decl {
  ast_node header;              ///< AST header
  bool is_static;               ///< true if the declaration is static
...
#define VARDECL_STATIC(p)       (((struct s_var_decl*) (p))->is_static)
...
void set_var_decl_static(ast_node *decl);
...
struct s_string_access {
  ...
  int slice;                    ///< access is a slice
  ast_node *index1;             ///< Low Index in the string
  ast_node *index2;             ///< Upper Index in the string
};

#define STRING_ACCESS_STR(p)    (((struct s_string_access *) p)->str)
#define STRING_ACCESS_SLICE(p)  (((struct s_string_access *) p)->slice)
#define STRING_ACCESS_INDEX1(p) (((struct s_string_access *) p)->index1)
#define STRING_ACCESS_INDEX2(p) (((struct s_string_access *) p)->index2)
  
ast_node *make_string_access(ast_node* name, int slice,
                             ast_node* index1, ast_node *index2);
...
struct s_function {
  ...
  char *c_body;                 ///< if not NULL, the string of the C body
...
void add_C_body_to_function(ast_node *func, char *body);
...
struct s_foreach_statement {
  ast_node header;              ///< AST header
  ast_node *type;               ///< type of the variable
  ast_node *var;                ///< name of the variable
  List lst;                     ///< List of values
  ast_node *body_stat;          ///< for body AST
};

ast_node *make_foreach_statement(ast_node *type, ast_node *var, List lst,
                             ast_node *body);
