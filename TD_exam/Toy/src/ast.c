 ...
 * Last file update:  3-Dec-2021 09:49 (eg)
 */
...
ast_node *make_identifier(char *id) {
  ...
  p->value = id;
  p->is_const = false; // by default variable is mutable
  return (ast_node *) p;
}
...
static void free_string_access(ast_node *node) {
  free_node(STRING_ACCESS_STR(node));
  free_node(STRING_ACCESS_INDEX1(node));
  free_node(STRING_ACCESS_INDEX2(node));
  free(node);
}

ast_node *make_string_access(ast_node* name, int slice,
                             ast_node* index1, ast_node* index2) {
  DEF_AST(p, string_access, NULL);

  STRING_ACCESS_STR(p)   = name;
  STRING_ACCESS_SLICE(p)  = slice;
  STRING_ACCESS_INDEX1(p) = index1;
  STRING_ACCESS_INDEX2(p) = index2;
  return (ast_node *) p;
}
...
ast_node *make_function(ast_node *name, ast_node *ret_type, List parameters) {
  ...
  p->c_body     = NULL; // will be filled later if a C function
  return (ast_node *) p;
}
...
/* ---- FOREACH ------------------------------------------------------------ */
static void free_foreach_statement(ast_node *node){
  struct s_foreach_statement *n = (struct s_foreach_statement *)node;
  free_node(n->type); free_node(n->var); free_node(n->body_stat);
  list_destroy(n->lst);
  free(node);
}
...
ast_node *make_foreach_statement(ast_node *type, ast_node *var, List lst,
                                 ast_node *body) {
  DEF_AST(p, foreach_statement, NULL);
  p->type      = type;
  p->var       = var;
  p->lst       = lst;
  p->body_stat = body;
