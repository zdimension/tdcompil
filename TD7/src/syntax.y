...
%right '=' AADD ASUB AMUL ADIV AMOD APOW
%right '?' ':'
%left KOR
%left KAND
%left '>' '<' GE LE
...
stmt:           ';'                     { $$ = make_expr_statement(NULL); }
        ...
        |       KTRY stmt %prec TRYX
                                        { $$ = make_try_statement($2, NULL, NULL); }
        |       KTRY stmt KCATCH stmt %prec TRYX
                                        { $$ = make_try_statement($2, $4, NULL); }
        |       KTRY stmt KFINALLY stmt %prec TRYX
                                        { $$ = make_try_statement($2, NULL, $4); }
        |       KTRY stmt KCATCH stmt KFINALLY stmt %prec TRYX
                                        { $$ = make_try_statement($2, $4, $6); }
...
expr:
        ...
        |       var AADD expr         { $$ = make_expression("+",  augassign, 2, $1,$3);}
        |       var ASUB expr         { $$ = make_expression("-",  augassign, 2, $1,$3);}
        |       var AMUL expr         { $$ = make_expression("*",  augassign, 2, $1,$3);}
        |       var ADIV expr         { $$ = make_expression("/",  augassign, 2, $1,$3);}
        |       var AMOD expr         { $$ = make_expression("%",  augassign, 2, $1,$3);}
        |       var APOW expr         { $$ = make_expression("**", augassign, 2, $1,$3);}
