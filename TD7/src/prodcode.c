...
void produce_code_expression(ast_node *node) {
  ...
  switch (EXPRESSION_ARITY(node)) {
    case 1:
      ...
      break;
    case 2:
      if (EXPRESSION_KIND(node)==augassign) {
        emit("(");
      ...
      if (EXPRESSION_KIND(node)==comp &&
        ...
        emit(") %s 0)", name);
          ...
          AST_TYPE(EXPRESSION_OP1(node))==string_type) {
        emit("_toy_strcat("); code(EXPRESSION_OP1(node));
        emit(", ");  code(EXPRESSION_OP2(node));
        emit(")");
      } else if (strcmp(name, "**") == 0)  {
        // power operator
...
void produce_code_switch_statement(ast_node *node) {
  ...
  List_item p = list_head(n->cond_list);
  bool first = true;
  while (p) {
    if (first)
      first = false;
    ...
    emit("if ("); code(list_item_data(p)); emit(")\n");
    p = list_item_next(p);
    indent(+1); code(list_item_data(p));
    p = list_item_next(p);
    indent(-1);
  }
  ...
  if (n->defcond) {
    if (!first)
      emit("else\n");
    indent(+1); code(n->defcond); indent(-1);
  }
}
...
void produce_code_try_statement(ast_node *node) {
  ...
  emit("{\n"); indent(+1);
  emit("TRY {\n"); 
  indent(+1); code(n->try); emit("\n");
  indent(-1); emit("} CATCH {\n"); 
  indent(+1); code(n->catch); emit("\n");
  indent(-1); emit("} FINALLY {\n"); 
  indent(+1); code(n->finally); emit("\n");
  indent(-1); emit("}\n");
  emit("ENDTRY;\n");
  emit("}\n"); indent(-1);
}
